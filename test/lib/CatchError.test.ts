...
describe("catchError (table-driven)", () => {
    it.each([
...

        {
            ...
            input: () => catchError(Promise.reject(new AnotherError("boom")), [TestError, AnotherError]),
    ...
    it.each([
        {
            ...
            executor: () =>
                catchError(Promise.reject(new AnotherError("unexpected")), [TestError]),
            expectedError: AnotherError,
        },
        {
            name: "does NOT catch errors when errorsToCatch is an empty array",
            executor: () => catchError(Promise.reject(new TestError("should rethrow")), []),
